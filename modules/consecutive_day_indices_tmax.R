...
consecutive_day_indices_tmax<-function(sDF,
    ...
    for (i in sccs.id) {
        ...
        if (is.na(sDF[sDF$sccs_id == i, "plant1_start"])) {
            ...
            ifelse(ghcn.l == 0, print(paste0("No GHCN station for SCCS ", i)), 
                   compute_consecutive_indices_1_tmax(s.doy, e.doy, ghcn.list, i, sourceDir, destDir))
            ...
            # compute consecutive days indices
            ifelse(ghcn.l == 0, print(paste0("No GHCN station for SCCS ", i)), 
                   compute_consecutive_indices_2_tmax(s1.doy, e1.doy, s2.doy, e2.doy,
            ...
            # compute consecutive days indices
            ifelse(ghcn.l == 0, print(paste0("No GHCN station for SCCS ", i)), 
                   compute_consecutive_indices_3_tmax(s1.doy, e1.doy, s2.doy, e2.doy, s3.doy, e3.doy,
                                                 ghcn.list, i, sourceDir, destDir))
        } else {
            ...
            # compute consecutive days indices
            ifelse(ghcn.l == 0, print(paste0("No GHCN station for SCCS ", i)), 
                   compute_consecutive_indices_4_tmax(s1.doy, e1.doy, s2.doy, e2.doy, 
