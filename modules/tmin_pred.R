...
##############################################################################################################
##Calculate TMIN predictability for whole temporal scale
tmin_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$tmin_spr, X$tmin_sum, 
                     X$tmin_aut, X$tmin_win)
        ...
        spr_cut = cut(X[, "tmin_spr"], breaks, include.lowest=TRUE,right=TRUE)
        sum_cut = cut(X[, "tmin_sum"], breaks, include.lowest=TRUE,right=TRUE)
        aut_cut = cut(X[, "tmin_aut"], breaks, include.lowest=TRUE,right=TRUE)
        win_cut = cut(X[, "tmin_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/tmin_PCM.csv"),row.names=F)
