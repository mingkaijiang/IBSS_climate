...
## Every 109 lines is a society
## Order of site in OSCCSID.txt
PDSI_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY) {   
    ...
    ### Read in PDSI data
    myDF <- read.table("data/PE_PDSI/SCCS_cru_pdsi_1901_2009.txt")
    ...
    for (j in 1:l) 
    {
        ...
        if (X[1,1] <= -10 | X[1,1]>= 10) {
            output[j,"P"] <- "NA"
            output[j,"C"] <- "NA"
            output[j,"M"] <- "NA"
        } else {
            bin <- matrix(0, ncol=14, nrow=12)
            dimnames(bin) <- list(NULL,c("bin_size","jan","feb","mar","apr",
                                         "may","jun","jul","aug",
                                         "sep","oct","nov","dec",
                                         "whole"))
            
            bin[,"bin_size"] <- c(seq(-5,5), 10)
            
            breaks = c(-10, seq(-5,5), 10)
    ...
    write.csv(output,paste0(destDir, "/PDSI_PCM.csv"),row.names=F)
