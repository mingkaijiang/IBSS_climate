...
R99PS_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        spr_cut = cut(X[, "r99p_spr"], breaks, include.lowest=TRUE,right=TRUE)
        sum_cut = cut(X[, "r99p_sum"], breaks, include.lowest=TRUE,right=TRUE)
        aut_cut = cut(X[, "r99p_aut"], breaks, include.lowest=TRUE,right=TRUE)
        win_cut = cut(X[, "r99p_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/R99PS_PCM.csv"),row.names=F)
