...
compute_consecutive_indices_no_growing_season_tmin <- function(wea.station, sccs.id, source, dest) {
    ...
    for (k in 1:length(wea.station)) 
    {
        ...
        for (j in outDF$year) {
            ...
            tmin10ann<-percentile(length(son$tmin),ann$tmin,0.1)/10.0
            ...
            cold_ann <- rle(ifelse((ann$tmin/10.0 - tmin10ann) < 0.0, 0, 1))
            ...
            outDF[outDF$year == j, "cold_ann"] <- max(cold_ann$lengths[cold_ann$values==0])
