...
##############################################################################################################
##Calculate tmax predictability for whole temporal scale
tmax_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$tmax_spr, X$tmax_sum, 
                     X$tmax_aut, X$tmax_win)
        ...
        spr_cut = cut(X[, "tmax_spr"], breaks, include.lowest=TRUE,right=TRUE)
        sum_cut = cut(X[, "tmax_sum"], breaks, include.lowest=TRUE,right=TRUE)
        aut_cut = cut(X[, "tmax_aut"], breaks, include.lowest=TRUE,right=TRUE)
        win_cut = cut(X[, "tmax_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/tmax_PCM.csv"),row.names=F)
