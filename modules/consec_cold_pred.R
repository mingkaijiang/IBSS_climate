...
consec_cold_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$cold_before, X$cold_growing, 
                     X$cold_after)
        ...
        if(diff > 0) {
            ...
            before_cut = cut(X[, "cold_before"], breaks, include.lowest=TRUE,right=TRUE)
            during_cut = cut(X[, "cold_growing"], breaks, include.lowest=TRUE,right=TRUE)
            after_cut = cut(X[, "cold_after"], breaks, include.lowest=TRUE,right=TRUE)
            ...
            col_sum <- sum(table(X[, "cold_before"]))
            whole_sum <- col_sum*3
