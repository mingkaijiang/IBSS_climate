...
RX1S_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        if(diff > 0) {
            ...
            spr_cut = cut(X[, "spr"], breaks, include.lowest=TRUE,right=TRUE)
            sum_cut = cut(X[, "sum"], breaks, include.lowest=TRUE,right=TRUE)
            aut_cut = cut(X[, "aut"], breaks, include.lowest=TRUE,right=TRUE)
            win_cut = cut(X[, "win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/RX1S_PCM.csv"),row.names=F)
