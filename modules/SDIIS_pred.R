...
SDIIS_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$sdii_spr, X$sdii_sum, 
                     X$sdii_aut, X$sdii_win)
        
        max_top <- round_any(max(v.range, na.rm=T), 10, f = ceiling)
        min_bot <- round_any(min(v.range, na.rm=T), 10, f = floor)
        ...
        if(diff > 0) {
            ...
            spr_cut = cut(X[, "sdii_spr"], breaks, include.lowest=TRUE,right=TRUE)
            sum_cut = cut(X[, "sdii_sum"], breaks, include.lowest=TRUE,right=TRUE)
            aut_cut = cut(X[, "sdii_aut"], breaks, include.lowest=TRUE,right=TRUE)
            win_cut = cut(X[, "sdii_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/SDIIS_PCM.csv"),row.names=F)
}
