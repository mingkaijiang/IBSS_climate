...
TH_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY) {   
    ...
    ### Read in PDSI data
    myDF <- read.table("data/PE_PDSI/craig_drought_th.txt")
    ...
    write.csv(output,paste0(destDir, "/TH_PCM.csv"),row.names=F)
