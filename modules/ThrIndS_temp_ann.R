##############################################################################################################
ThrIndS_temp_ann <- function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY) {
...
#                        & dd$tmin>=1,"tmin"]
        ...
        tmintmp_ann<-dd[dd$year >= startyear 
                        & dd$year <= endyear, "tmin" ]
        #                        & dd$tmin>=1,"tmin"]
        
        tmintmp_ann<-tmintmp_ann[is.na(tmintmp_ann)==F]
        ...
        len_ann<-length(tmintmp_ann)
        
        
        ys<-yeare-years+1
        dp<-matrix(0,ys,6)
        dimnames(dp)<-list(NULL,c("year","tmin_spr","tmin_sum","tmin_aut","tmin_win","tmin_ann"))
        dp[,"year"]<-years:yeare
        ...
        for(i in years:yeare)
        {
          ...
          dp[(i-years+1),"tmin_spr"]<-min(dd$tmin[dd$year == i & dd$month >= 3 & dd$month<= 5],na.rm=T)/10.0
         
          if(inName == "data/ghcnd_gap_filled_tmin/CA001054920.csv" && i == 1898) { print(dd$tmin[dd$year == i & dd$month >= 9 & dd$month<= 11],na.rm=T)}
          dp[(i-years+1),"tmin_sum"]<-min(dd$tmin[dd$year == i & dd$month >= 6 & dd$month<= 8],na.rm=T)/10.0
          dp[(i-years+1),"tmin_aut"]<-min(dd$tmin[dd$year == i & dd$month >= 9 & dd$month<= 11],na.rm=T)/10.0
          dp[(i-years+1),"tmin_win"]<-min(dd$tmin[dd$year == i & (dd$month == 12 | dd$month== 1 | dd$month==2)],na.rm=T)/10.0
          dp[(i-years+1),"tmin_ann"]<-min(dd$tmin[dd$year == i],na.rm=T)/10.0
        }
        dp<-as.data.frame(dp)
        ...
        target<-as.data.frame(cbind(dp,fd_spr,fd_sum,fd_aut,fd_win,fd_ann))
        ...
        for (year in years:yeare) {
            ...
            mid_ann<-dd[dd$year==year,"tmin"]
            mid_ann<-mid_ann[is.na(mid_ann)==F]
            target[target$year==year,"fd_ann"]<-length(mid_ann[mid_ann/10.0<=0.0])
