...
consec_wet_pred_hunter_gatherer<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        if(diff > 0) {
            ...
            djf_cut = cut(X[, "wet_DJF"], breaks, include.lowest=TRUE,right=TRUE)
            mam_cut = cut(X[, "wet_MAM"], breaks, include.lowest=TRUE,right=TRUE)
            jja_cut = cut(X[, "wet_JJA"], breaks, include.lowest=TRUE,right=TRUE)
            son_cut = cut(X[, "wet_SON"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    write.csv(output,paste0(destDir, "/Consec_wet_PCM_hunter_gatherer.csv"),row.names=F)
