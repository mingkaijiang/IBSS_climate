...
consec_warm_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    ...
    for (j in 1:length(DatFiles)) 
    {
        ...
        v.range <- c(X$warm_before, X$warm_growing, 
                     X$warm_after)
        ...
        if(diff > 0) {
            ...
            before_cut = cut(X[, "warm_before"], breaks, include.lowest=TRUE,right=TRUE)
            during_cut = cut(X[, "warm_growing"], breaks, include.lowest=TRUE,right=TRUE)
            after_cut = cut(X[, "warm_after"], breaks, include.lowest=TRUE,right=TRUE)
            ...
            col_sum <- sum(table(X[, "warm_before"]))
            whole_sum <- col_sum*3
    ...
    write.csv(output,paste0(destDir, "/Consec_wsdi_PCM.csv"),row.names=F)
