...
consecutive_day_indices_5_stations<-function(sDF,
    ...
    ### prepare sDF
    sDF$ghcn1 <- as.character(sDF$ghcn1)
    sDF$ghcn2 <- as.character(sDF$ghcn2)
    sDF$ghcn3 <- as.character(sDF$ghcn3)
    sDF$ghcn4 <- as.character(sDF$ghcn4)
    sDF$ghcn5 <- as.character(sDF$ghcn5)
    ...
    for (i in sccs.id) {
        ...
        if (is.na(sDF[sDF$sccs_id == i, "plant1_start"])) {
            ...
            # compute consecutive days indices
            ifelse(ghcn.l == 0, print(paste0("No GHCN station for SCCS ", i)), 
                   compute_consecutive_indices_2(s1.date=s1.doy, e1.date=e1.doy, 
                                                 s2.date=s2.doy, e2.date=e2.doy,
                                                 wea.station=ghcn.list, sccs.id=i, 
