...
R10S_pred<-function(sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
  ...
  for (thisFile in 1:length(DatFiles)) 
  {
    ...
    biome_prcp <- read.table("E:/IBSS/Output/biome_prcp_rng/biome_R10S.csv",header=T,sep=",")
    X <- read.table(inName, sep=",", header=T,quote="")
    ...
    spr_cut = cut(X[X$WWF_MHTNUM == i, "R10_spr"], breaks, include.lowest=TRUE,right=TRUE)
    sum_cut = cut(X[X$WWF_MHTNUM == i, "R10_sum"], breaks, include.lowest=TRUE,right=TRUE)
    aut_cut = cut(X[X$WWF_MHTNUM == i, "R10_aut"], breaks, include.lowest=TRUE,right=TRUE)
    win_cut = cut(X[X$WWF_MHTNUM == i, "R10_win"], breaks, include.lowest=TRUE,right=TRUE)
    ...
    col_sum <- sum(table(X[X$WWF_MHTNUM == i, "R10_spr"]))
    whole_sum <- col_sum*4
