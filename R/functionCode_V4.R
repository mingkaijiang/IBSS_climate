...
##############################################################################################################
### Gap filling using the hyfo package as an easy solution
Gap_Fill_2 <- function(stationDF = STATION.DATAFRAME, threshold, 
                     sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY) {
    
    dir.create(destDir, showWarnings = FALSE)
    
    targList <- paste0(stationDF$ghcn2,".csv")
    supList2 <- paste0(stationDF$ghcn3,".csv")
    supList3 <- paste0(stationDF$ghcn4,".csv")
    supList4 <- paste0(stationDF$ghcn5,".csv")
    supList5 <- paste0(stationDF$ghcn6,".csv")
    supList6 <- paste0(stationDF$ghcn7,".csv")
    supList7 <- paste0(stationDF$ghcn8,".csv")
    supList8 <- paste0(stationDF$ghcn9,".csv")
